...
class KQNodes(SIA):
    ...
    def calculate_cause_repertoire(
        ...
        all_mech = sorted(set(
            idx for group in groups for t, idx in group if t == ACTUAL
        ))
        all_purv = sorted(set(
            idx for group in groups for t, idx in group if t == EFFECT
        ))
        ...
        likelihood_all = np.ones(n_states, dtype=np.float64)
        for i in all_mech:
            mi = int(estado[i])
            if mi == 1:
                likelihood_all *= tpm[:, i]
            else:
                likelihood_all *= (1.0 - tpm[:, i])
        ...
        state_bits = np.arange(n_states, dtype=np.uint64)
        ...
        for p_idx, p in enumerate(all_purv):
            ...
            if not mech_for_p:
                part[p_idx] = 0.5
                continue
            likelihood_g = np.ones(n_states, dtype=np.float64)
            for i in mech_for_p:
                mi = int(estado[i])
                if mi == 1:
                    likelihood_g *= tpm[:, i]
                else:
                    likelihood_g *= (1.0 - tpm[:, i])
